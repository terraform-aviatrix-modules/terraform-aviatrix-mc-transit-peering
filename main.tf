locals {
  ...
  transit_gateways_sorted = try(keys(var.transit_gateways_with_local_as), sort(var.transit_gateways))
  ...
  peerings = flatten([
    for gw in local.transit_gateways_sorted : [
      ...
      for peer_gw in slice(local.transit_gateways_sorted, index(local.transit_gateways_sorted, gw) + 1, length(local.transit_gateways_sorted)) : {
        gw1 = gw
        gw2 = peer_gw
      }
    ]
  ...
  peerings2 = {
    for key1, value1 in local.peerings_map : key1 => {
      ...
      # 3) none as fallback 
      prepend_as_path1 = try([for i in range(local.modification_1[key1]["gw1"]) : var.transit_gateways_with_local_as[value1["gw1"]]], try([for i in range(var.full_mesh_prepending) : var.transit_gateways_with_local_as[value1["gw1"]]], null)),
      prepend_as_path2 = try([for i in range(local.modification_1[key1]["gw2"]) : var.transit_gateways_with_local_as[value1["gw2"]]], try([for i in range(var.full_mesh_prepending) : var.transit_gateways_with_local_as[value1["gw2"]]], null))
    }
  }
